...
import BlockchainJobActions from "../components/BlockchainJobActions";
...
const JobDetail = () => {
	...
	const handleApplyJob = async (e) => {
		...
		// Check if freelancer has connected wallet
		if (!user.walletAddress && !walletAddress) {
			setApplyError("Vui lòng kết nối ví MetaMask trước khi ứng tuyển.");
			return;
		}
		...
		try {
			// Use wallet from user profile or newly connected wallet
			...
			if (err.response) {
				if (
					...
					setApplyError(
						err.response.data.message || "Lỗi không xác định khi ứng tuyển.",
					);
				}
			} else {
	...
	return (
		<MainLayout>
			<div className="max-w-5xl mx-auto">
				<ReusableHeading
					title={job.title}
					className="text-4xl text-blue-800 border-b pb-3 mb-6"
				/>
				...
				<div className="grid md:grid-cols-3 gap-8">
							...
							<div className="flex flex-wrap gap-3">
								{job.skillsRequired && job.skillsRequired.length > 0 ? (
									...
									<p className="text-gray-500">
										Không có yêu cầu kỹ năng cụ thể nào được liệt kê.
									</p>
								)}
							</div>
						</section>
					...
					<div className="md:col-span-1 space-y-6">
							...
							<div className="space-y-3 text-gray-700">
								...
								<p className="flex items-center">
									<FontAwesomeIcon
										...
										className={`mr-3 ${currentStatus.color}`}
									/>
									Trạng thái:
									<span
										className={`ml-auto font-semibold ${currentStatus.color} px-2 py-0.5 rounded-lg`}
									>
										{currentStatus.label}
									</span>
								</p>
						...
						{isJobOpen && (
							<div className="bg-white p-6 rounded-xl shadow-lg border border-indigo-200">
								...
								{applicationStatus.message && (
									<div
										className={`p-3 mb-4 text-sm rounded-lg ${applicationStatus.isApplied ? "text-green-700 bg-green-100" : "text-blue-700 bg-blue-100"}`}
									>
										{applicationStatus.message}
									</div>
								)}
										...
										{!user.walletAddress && (
											<div className="mb-4 p-4 bg-purple-50 border border-purple-200 rounded-lg">
												<p className="text-sm font-semibold text-purple-800 mb-3">
													🔐 Bước 1: Kết nối ví Blockchain
												</p>
												<p className="text-xs text-purple-700 mb-3">
													Để tham gia các dự án có thanh toán blockchain, bạn
													cần kết nối ví MetaMask.
												</p>
										...
										<div className="mb-4">
											...
											>
												{user.walletAddress
													...
													: "Bước 2: Thư xin việc (Cover Letter):"}
											</label>
											<textarea
												id="coverLetter"
												rows="5"
						...
						{/* Arbitrator Panel - Always show if job is in dispute */}
						{job.escrowAddress && <ArbitratorPanel job={job} />}
					</div>
				</div>
			</div>
		</MainLayout>
