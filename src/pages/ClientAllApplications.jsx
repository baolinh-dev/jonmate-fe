...
import { 
    faFileAlt, faDollarSign, faCalendarAlt, 
    faUser, faEnvelope, faBriefcase, 
    faHourglassHalf, faCheckCircle, faTimesCircle, faSpinner, faThumbsUp, faThumbsDown, faPaperPlane 
...
const ApplicationOverviewCard = ({ application, onStatusUpdate }) => {
    ...
    const statusConfig = {
        applied: { label: 'Đã ứng tuyển', color: 'bg-yellow-100 text-yellow-700', icon: faPaperPlane },
        accepted: { label: 'Đã chấp nhận', color: 'bg-green-100 text-green-700', icon: faCheckCircle },
        rejected: { label: 'Đã từ chối', color: 'bg-red-100 text-red-700', icon: faTimesCircle },
    };
    ...
    return (
        <div className="bg-white p-5 rounded-xl shadow-lg border border-gray-100 transition duration-300 hover:shadow-xl hover:border-indigo-300">
            <div className="flex justify-between items-start border-b pb-3 mb-3">
                ...
                {/* Trạng thái Hồ sơ */}
                <span className={`mt-2 md:mt-0 px-3 py-1 text-xs font-bold uppercase rounded-full ${currentStatus.color} flex items-center whitespace-nowrap`}>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-4">
                <p className="flex items-center">
                    ...
                    Ngân sách Job: <span className="font-bold text-green-700 ml-1">{formattedBudget}</span>
                </p>
                <p className="flex items-center">
                    <FontAwesomeIcon icon={faCalendarAlt} className="mr-2 text-gray-500" />
                    Ứng tuyển vào: {new Date(application.submittedAt || application.createdAt).toLocaleDateString("vi-VN")}
                </p>
            </div>
            ...
            <div className="pt-2 border-t flex justify-end space-x-3">
                {isApplied ? ( // Hiển thị nút khi trạng thái là 'applied'
                    <>
                        ...
                        >
                            {isUpdating ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faThumbsDown} className="mr-2" />}
                            Từ chối
                        </button>
                        <button 
                        ...
                        >
                            {isUpdating ? <FontAwesomeIcon icon={faSpinner} spin className="mr-2" /> : <FontAwesomeIcon icon={faThumbsUp} className="mr-2" />}
                            Chấp nhận
                        </button>
                    </>
                ) : (
                    <span className="text-sm italic text-gray-500">Đã xử lý hồ sơ này.</span>
                )}
            </div>
...
const ClientAllApplications = () => {
    ...
    const handleUpdateStatus = async (applicationId, newStatus) => {
        ...
        try {
            ...
            const errMsg = err.response?.data?.message || 'Đã xảy ra lỗi khi cập nhật trạng thái.';
        ...
        const fetchAllApplications = async () => {
            ...
            try {
                ...
                if (err.response && err.response.status === 403) {
                    setError("Bạn không có quyền truy cập trang này. Vui lòng đăng nhập bằng tài khoản Client.");
                } else {
        ...
        return (
            <MainLayout>
                <div className="text-center py-20 text-lg font-medium text-indigo-600">
                    <div className="animate-spin inline-block w-8 h-8 border-[3px] border-current border-t-transparent text-indigo-500 rounded-full" role="status"></div>
    ...
    const statusClass = statusMessage?.startsWith('Lỗi:') ? 'text-red-700 bg-red-100' : 'text-blue-700 bg-blue-100';
        ...
        <MainLayout>
            <div className="max-w-5xl mx-auto">
                ...
                <p className="text-lg font-medium mb-6 text-gray-700">
                    <FontAwesomeIcon icon={faBriefcase} className="mr-2 text-indigo-500" />
                    Tổng số đơn ứng tuyển: <span className="font-extrabold text-indigo-600">{applicationCount}</span>
                </p>

                {/* Thông báo cập nhật trạng thái */}
                {statusMessage && (
                    <div className={`p-4 mb-6 text-sm rounded-lg ${statusClass}`} role="alert">
                        {statusMessage}
                    </div>
                )}
                ...
                <div className="space-y-6">
                    {applicationCount > 0 ? (
                        ...
                        <div className="bg-gray-50 p-8 rounded-xl text-center border border-dashed border-gray-300">
                            <h3 className="text-xl font-semibold text-gray-600">Bạn chưa nhận được đơn ứng tuyển nào.</h3>
                            <p className="text-gray-500 mt-2">Hoặc bạn chưa đăng công việc nào.</p>
                        </div>
                    )}
                </div>
